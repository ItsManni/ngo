    ...
    public function GetAllProductBySubcategoryID($subcatID)
{
   ...
   public function GetSubcategoriesByPageUrl($pageUrl)
    {
        
        $pageUrl = mysqli_real_escape_string($this->conn, $pageUrl);
        $sqlCategory = "SELECT ID FROM sevaye WHERE PageUrl = '$pageUrl' AND IsActive = 1 LIMIT 1";
        $resultCategory = mysqli_query($this->conn, $sqlCategory);
        if ($resultCategory && mysqli_num_rows($resultCategory) > 0) {
            $row = mysqli_fetch_assoc($resultCategory);
            $categoryID = intval($row['ID']);
            $where = "WHERE IsActive=1 AND CategoryID = $categoryID ORDER BY Priority ASC";
