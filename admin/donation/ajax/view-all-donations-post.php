...
$searchQuery = " ";
if($searchValue != ''){
   $searchQuery = " and (Name like '%".$searchValue."%' Email like '%".$searchValue."%' Phone like '%".$searchValue."%')";
}
...
$totalRecordwithFilter = $core->_getTotalRows($conn,'donations', $filter);
## Total number of record with filtering
$totalRecords = $Enquiries->getTotalEnquiries('donations');
$filter = $filter." ORDER BY ".$columnName." ".$columnSortOrder;
$filter = $filter." limit ".$row.",".$rowperpage;
$enquiries_details = $core->_getTableRecords($conn,'donations', $filter);
...
foreach ($enquiries_details as $enquiries_detail)
{
  ...
  $data[] = array(
    "id"=>$ID,
    "Name"=>$Name,
    "EmailPhoneNumber"=>$Email.'<br>'.$Phone,
